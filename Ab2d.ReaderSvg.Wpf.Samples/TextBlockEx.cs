...
namespace Ab2d.Samples.ReaderSvgSamples
{
    ...
    /// </para>
    /// <para>
    /// To add <strong>hyperlink</strong> insert "\@" that is followed by anchor text, then add ':' character and then url address. Complete with '|' character into the text - for example: "click here \@Ab3d.PowerToys:http://www.ab4d.com/PowerToys.aspx| to learn more"
    /// </para>
    /// </remarks>
    ...
    public class TextBlockEx : TextBlock
    {
        ...
        {
            this.BeginInit(); // We cannot put that into UpdateText, because UpdateText is also called from OnContentTextChanged that is already inside Begin/EndInit

            if (!string.IsNullOrEmpty(this.Text))
                UpdateText(this.Text);
            else
                UpdateText(this.ContentText);
